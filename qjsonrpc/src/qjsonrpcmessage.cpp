...
QJsonRpcMessage QJsonRpcMessagePrivate::createBasicRequest(const QString &method, const QVariantList &params)
{
    ...
    if (!params.isEmpty()) {
        QJsonArray args(QJsonArray::fromVariantList(params));
        QVariantList::const_iterator it(params.constBegin());

        for (int i = 0; it != params.end(); ++it, ++i) {
            if (it->type() >= QVariant::UserType) {
                QJsonValue value;

                QJsonRpcConverter::toJson(value, it->userType(), it->constData());
                if (value.isUndefined()) {
                    qWarning("QJsonRpcMessage: failed to convert UserType argument calling %s",
                            qPrintable(method));
                }
                else
                    args.replace(i, value);
            }
        }

        request.d->object->insert("params", args);
    }
...
QJsonRpcMessage QJsonRpcMessage::createResponse(const QVariant &result) const
{
    ...
    if (d->object->contains("id")) {
        ...
        if (result.type() == QVariant::UserType)
        {
            QJsonValue val;
            QJsonRpcConverter::toJson(val, result.userType(), result.data());

            if (!val.isUndefined())
                object->insert("result", val);
            else
...
QJsonArray QJsonRpcMessage::rawParams() const
{
    if (d->type == QJsonRpcMessage::Response || d->type == QJsonRpcMessage::Error)
        return QJsonArray();
    if (!d->object)
...
QVariant QJsonRpcMessage::result(int userType) const
{
    if (d->type != QJsonRpcMessage::Response || !d->object)
        return QVariant();
    ...
    {
        QVariant ret(userType, (const void *) NULL);
        QJsonValue json = d->object->value("result");

        QJsonRpcConverter::fromJson(json, userType, ret.data());
        return ret;
    }
    else
